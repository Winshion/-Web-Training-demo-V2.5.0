...
def format_dir_wordcloud_pye(job_csv, job):
    ...
    # wc_lst就是需要的数据
    # 接下来进行词云图的构建
    dir_wc = (WordCloud(init_opts=opts.InitOpts(bg_color='#777', page_title="公司经营方向词云",
                                                width='100%', height='500px',theme=ThemeType.DARK))
              .add('', wc_lst, shape='cardioid', word_size_range=[20, 250],
                   height=400, width=800, pos_top=3, pos_bottom=3, pos_left='6%', pos_right='5%',
...
def education_pie_pye(job_csv, job):
    ...
    p = (Pie(init_opts=opts.InitOpts(bg_color="#777", theme=ThemeType.DARK,
...
def format_bubble_plot_pye(job_csv, job):
    ...
    slry = (
        Bar(opts.InitOpts(
            theme=ThemeType.DARK, bg_color='#777',
    ...
    scale = (
        Bar(opts.InitOpts(
            theme=ThemeType.DARK, bg_color='#777'))  # 创建散点图对象
        .add_xaxis(xaxis_data=txt)  # 添加x轴数据（公司规模/人）
        .add_yaxis(
            series_name='公司规模/人',
...
def store_plot_pye(job_csv, job):
    ...
    c = Boxplot(
        init_opts=opts.InitOpts(bg_color="#777", width="100%",
                                height="500px", theme=ThemeType.DARK))
    c.add_xaxis(xaxis_data=x_axis)
    c.add_yaxis(None, c.prepare_data(y_axis),
                itemstyle_opts=opts.ItemStyleOpts(color="#777", border_color='cyan'))
    c.set_global_opts(visualmap_opts=opts.VisualMapOpts(is_show=False, type_="type"),
                      yaxis_opts=opts.AxisOpts(name="薪资", axislabel_opts=opts.LabelOpts(formatter="{value} 千元/月")),
